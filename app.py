...
def process_document(uploaded_file: UploadedFile) -> List[Document]:
    ...
    """
    # First try to load directly from bytes without temp file
    try:
        pdf_stream = BytesIO(uploaded_file.getvalue())
        loader = PyMuPDFLoader(file_path=pdf_stream)
        docs = loader.load()
    except Exception:
        # Fall back to temp file approach if direct loading fails
        try:
            ...
            # Ensure temp file is deleted even if loading fails
            try:
                os.unlink(temp_path)
